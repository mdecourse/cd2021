...
def track_blue_object(image):
    ...
    for i in range(len(contours)):
        mc[i] = (mu[i]['m10'] / (mu[i]['m00']+0.000001), mu[i]['m01'] / (mu[i]['m00'])+0.000001)
    return mc
#影像尋找紅色物件
    ...
    if err == vrep.simx_return_ok:
        ...
        if ret_green != None:
            ...
            for i in range(len(ret_red)):
                if ret_red[i][1] >=30 and ret_red[i][1] <=52:
                    red00 = (ret_red[i][0], ret_red[i][1])
                elif ret_red[i][1] >= 53 and ret_red[i][1] <=75:
            ...
            if blue00 != None and red00 != None:
                ...
                if Rv<0.0:
                    ...
                    pass
            #藍桿一移動
            if  blue10 != None and blue11 != None:
            ...
            print(ret_green)
            #藍桿一踢球
            if ret_green[1] <= 200 and ret_green[1] >= 183:
                speed(BRev0_handle,B_KickBallVel)
            else:
                speed(BRev0_handle,R_KickBallVel)
            #紅桿一踢球
            if ret_green[1] <= 75 and ret_green[1] >= 53:
